...
const thbtn = document.querySelector("#thbtn");
let quality = "360p";
let response;
let fileSize;
let video_id;
...
if (bodytube) {
  ...
  runButton.addEventListener("click", async () => {
    ...
    if (!isValidYoutubeUrl(inp.value)) {
      ...
      try {
        response = await fetch(`/d?val=${inp.value}&reso=${quality}`, {
          method: "POST",
        });
        ...
        if (!response.ok && response.status == 500) {
          showMsg(
            errMsg,
            `<i class="fa-regular fa-circle-xmark"></i> Can not download video ${response.status}`,
            "red"
          );
  ...
  inp.addEventListener("keyup", async function () {
    ...
    try {
      response = await fetch(`/size?value=${inp.value}`, {
        method: "GET",
        cache: "no-store",
      });

      fileSize = (await response.json()).size;
      loadingInfo.classList.add("hidden");
      if (fileSize.includes("Error")) {
        sizeDiv.innerHTML = `<i class="fa-solid fa-circle-exclamation"></i> can not get video size ${fileSize}`;
      } else {
...
function rnum(rangeNumber = 10) {
  return Math.floor(Math.random() * rangeNumber);
}

let users = ["Fady", "Abanoub", "Jerom", "Emad", "Hend"];
let prices = ["$100", "$1", "$2000", "$50"];

console.log(
  `User: ${users[rnum(users.length)]} , you won ${prices[rnum(prices.length)]}`
);
